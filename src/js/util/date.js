...
const dateSort = (dateA, dateB) => {
  if (moment(dateA).isBefore(dateB, 'day')) return -1
  if (moment(dateA).isAfter(dateB, 'day')) return 1
  return 0
}

const timeParse = t => moment(t, 'HH:mm')
const dateTimeParse = (d, t) => moment(`${d} ${t}`, 'YYYY-MM-DD HH:mm')
...
const dateWithTimeSort = (dateA, timeA, dateB, timeB) => {
  const datetimeA = dateTimeParse(dateA, timeA)
  const datetimeB = dateTimeParse(dateB, timeB)
  if (datetimeA.isBefore(datetimeB)) return -1
  if (datetimeA.isAfter(datetimeB)) return 1
  return 0
}
